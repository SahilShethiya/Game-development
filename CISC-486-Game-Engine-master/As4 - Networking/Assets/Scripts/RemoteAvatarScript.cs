...
public class RemoteAvatarScript : MonoBehaviour {
	...
	void MoveAvatarWithDeadReckoning() {
		if (_hasTargetPosition) {
			if (transform.position != targetPosition) {
				transform.position = Vector3.MoveTowards(transform.position, targetPosition, AnimationInfo.fastWalkSpeed*Time.deltaTime);
			} else {
				transform.position += (targetPosition - _lastPosition).normalized * AnimationInfo.walkSpeed*Time.deltaTime;
			}
		}
		if (_hasTargetRotation) {
			if (transform.rotation != targetRotation) {
				...
				transform.rotation = Quaternion.SlerpUnclamped(_lastRotation, targetRotation, AnimationInfo.turnSpeed*Time.deltaTime);
			}
		}
	}
	...
	void SmoothlyCorrectAvatarPosition() {
		if (_hasTargetPosition) {
			transform.position = Vector3.MoveTowards(transform.position, targetPosition, AnimationInfo.walkSpeed*Time.deltaTime);
		}
		if (_hasTargetRotation) {
			transform.rotation = Quaternion.RotateTowards(transform.rotation, targetRotation, AnimationInfo.turnSpeed*Time.deltaTime);
		}
