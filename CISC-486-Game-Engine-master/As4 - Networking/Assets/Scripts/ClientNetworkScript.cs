...
public class ClientNetworkScript : MonoBehaviour {
	...
	// Fill in the IP of the server. Use "127.0.0.1" if the server is located
	// on the same computer as the clients
	string _serverHostIP = "127.0.0.1";

	bool _isConnected = true;


	// To send messages, we will need a socket (host id), a connection id (reference to server),
	// and a channel id (for lossless/lossy communication.) These are all set when this client is initialized.
	int _hostId;
	int _connectionId;
	...
	LocalAvatarScript _localAvatar;
	RemoteAvatarScript _remoteAvatar;
	byte _localAvatarId;
	...
	void InitNetwork() {
		...
		Debug.LogFormat("Establishing network connection with hostId {0}; connectionId {1}", _hostId, _connectionId);
	}
	...
	void InitializeAvatars(byte localAvatarId) {
		...
		if(localAvatarId == Messages.flora) {
			Debug.Log("Initializing Flora as local");
			_localAvatar = floraAvatar.GetComponent<LocalAvatarScript>();
			_remoteAvatar = tommyAvatar.GetComponent<RemoteAvatarScript>();
		} else {
			Debug.Assert(localAvatarId == Messages.tommy);

			Debug.Log("Initializing Tommy as local");
			_localAvatar = tommyAvatar.GetComponent<LocalAvatarScript>();
			_remoteAvatar = floraAvatar.GetComponent<RemoteAvatarScript>();
		}
	...
	void UpdateRemoteAvatar(byte[] message) {
		...
		_remoteAvatar.targetPosition = new Vector3(x, _remoteAvatar.transform.position.y, z);
		_remoteAvatar.targetRotation = Quaternion.AngleAxis(r, Vector3.up);
		_remoteAvatar.targetMovementState = m;
	}
	...
	void ProcessIncomingMessages() {
		...
		if(_isConnected) {
			...
			while(messagesAvailable && Time.realtimeSinceStartup - startTime < Time.maximumDeltaTime) {
				...
				case NetworkEventType.ConnectEvent:
					Debug.Log(string.Format("Connection received from host {0}, connectionId {1}, channel {2}",
						recHostId, connectionId, channelId));
					Messages.LogConnectionInfo(_hostId, connectionId);
					break;
				case NetworkEventType.DataEvent:
					Debug.Log(string.Format("Message received from host {0}, connectionId {1}, channel {2}",
						recHostId, connectionId, channelId));
					ProcessMessage(recBuffer, dataSize);
					break;
				case NetworkEventType.DisconnectEvent:
					Debug.Log(string.Format("Disconnection received from host {0}, connectionId {1}, channel {2}",
						recHostId, connectionId, channelId));
					_isConnected = false;
					break;
				}
	...
	void SendMessageToServer(byte[] message, int channelId) {
		...
		//Debug.Log("Outgoing message: "+byteArrayToString(message));
	...
	/// <summary>
	/// Sets the prediction algorithm to none (no prediction), dead reckoning, or dead reckoning with smooth corrections.
	...
	public void SetAlgorithm(int algorithmIndex) {
		Debug.LogFormat("Algorithm index: {0}", algorithmIndex);
		Debug.Assert(_remoteAvatar != null);
		_remoteAvatar.UpdateAlgorithm = (RemoteAvatarScript.Algorithm)algorithmIndex;
	}
